...
class inspectController extends Controller
{
    ...
    public function changeDataInspeksi(Request $request)
    {
        ...
        // dd($rekap[2]['BKE']);
        return view('Qcinspeksi.dataInspeksi', $result);
    }
    ...
    public function getDataDay(Request $request)
    {
        ...
        $result = rekap_blok($est, $afd, $date, $reg);
    ...
    public function excelqcinspeksi(Request $request)
    {
        $date = $request->input('getregional');
        $reg = $request->input('getdate');
        return Excel::download(new Exportqcinspeksi($date, $reg), 'Excel QC Inspeksi Regional-' . $date . '-' . 'Bulan-' . $reg . '.xlsx');
    }
    ...
    public function verifdata(Request $request)
    {
        ...
        // dd($status);
        if ($menu === 'sidaktph' || $menu === 'sidakmutubuah') {
            if ($status->isEmpty()) {
                ...
                if ($verifby_askep || $verifby_manager) {
                    return response()->json('all_approved');
                } elseif (!$verifby_askep || !$verifby_manager) {
                    return response()->json('not_approved_all');
                } else {
