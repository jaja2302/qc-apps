...
class RekapController extends Controller
{
    ...
    public function getdatayear(Request $request)
    {
        ...
        $cacheKeyRekapQc = 'rekap_qc_' . $tahun . '_' . $regional;
        $cacheKeyRekapTph = 'rekap_tph_' . $tahun . '_' . $regional;
        $cacheKeyRekapMutubuah = 'rekap_mutubuah_' . $tahun . '_' . $regional;

        // Fetch and store other data in cache
        $rekap_qc = Cache::remember($cacheKeyRekapQc, $this->minutes, function () use ($tahun, $regional) {
            return rekap_pertahun($tahun, $regional);
        });

        $rekap_tph = Cache::remember($cacheKeyRekapTph, $this->minutes, function () use ($tahun, $regional) {
            return rekap_pertahun_sidaktph($regional, $tahun, null, null);
        });

        $rekap_mutubuah = Cache::remember($cacheKeyRekapMutubuah, $this->minutes, function () use ($tahun, $regional) {
            return rekap_pertahun_mutubuah($regional, $tahun);
        });
        $resultafdeling_qc = $rekap_qc['resultafdeling'];
        // dd($resultafdeling_qc, $estate);

        $rekap_qc_afdeling = [];
        foreach ($resultafdeling_qc as $key => $value) {
            foreach ($estate as $key1 => $wil) {
                foreach ($wil as $key2 => $est) {
                    if ($key2 === $key) {
                        $rekap_qc_afdeling[$key1][$key] = $value;
                    }
                }
            }
        }

        foreach ($rekap_qc_afdeling as $key => $value) {
            foreach ($value as $key1 => $value1) {
                foreach ($value1 as $key2 => $value2) {
                    $pokok_panenEst = 0;
                    $jum_haEst =  0;
                    ...
                    $sum_rstEst = 0;
                    foreach ($value2 as $key3 => $value3) {
                        $pokok_panenEst += $value3['pokok_samplecak'];
                        $jum_haEst += $value3['ha_samplecak'];
                        $janjang_panenEst += $value3['jumlah_panencak'];
                        $p_panenEst += $value3['pcak'];
                        $k_panenEst += $value3['kcak'];
                        $brtgl_panenEst += $value3['tglcak'];
                        $bhtsEST   += $value3['bhts_scak'];
                        $bhtm1EST += $value3['bhtm1cak'];
                        $bhtm2EST   += $value3['bhtm2cak'];
                        $bhtm3EST   += $value3['bhtm3cak'];
                        $pelepah_sEST += $value3['palepah_pokokcak'];

                        // mutu buah 
                        $jum_haEst_buah += $value3['tph_baris_bloksbh'];
                        $sum_SamplejjgEst += $value3['sampleJJG_totalbh'];
                        $sum_bmtEst += $value3['total_mentahbh'];
                        $sum_bmkEst += $value3['total_overbh'];
                        $sum_overEst += $value3['total_abnormalbh'];
                        $sum_abnorEst += $value3['total_abnormalbh'];
                        $sum_kosongjjgEst += $value3['total_jjgKosongbh'];
                        $sum_vcutEst += $value3['total_vcutbh'];
                        $sum_krEst += $value3['jum_krbh'];
                        // mutu transpat 
                        $sum_btEst += $value3['total_brdtrans'];
                        $sum_rstEst += $value3['total_buahtrans'];
                        $dataBLokEst += $value3['tph_sampleNew'];
                    }
                    ...
                    // mutu buah 
                    if ($sum_krEst != 0) {
                        $total_krEst = $sum_krEst / $jum_haEst_buah;
                    } else {
                        $total_krEst = 0;
                    }
                    ...
                    $totalskormtbuah =   skor_buah_mentah_mb($PerMthEst) + skor_buah_masak_mb($PerMskEst) + skor_buah_over_mb($PerOverEst) + skor_jangkos_mb($PerkosongjjgEst) + skor_vcut_mb($PerVcutest) + skor_abr_mb($per_krEst);

                    // mutu transport 
                    $brdPertphEst = ($dataBLokEst != 0) ? round($sum_btEst / $dataBLokEst, 3) : 0;
                    $buahPerTPHEst = ($dataBLokEst != 0) ? round($sum_rstEst / $dataBLokEst, 3) : 0;
                    $totalskormttrans = skor_brd_tinggal($brdPertphEst) + skor_buah_tinggal($buahPerTPHEst);
                    $nonZeroValues = array_filter([$sum_btEst, $sum_rstEst]);
                    ...
                    $rekap_qc_afdeling[$key][$key1][$key2]  = [
                        ...
                        // 'tph_baris_bloksbh' => $jum_haEst_buah,
                        ...
                        // 'totalSkortrans' => $totalskormttrans,
                        // 'mututrans' => '-----------------------------------',
                        'totalskorinspeksi' => $totalSkorEst + $totalskormtbuah + $totalskormttrans,
        ...
        // dd($rekap_qc_afdeling);
        foreach ($rekap_tph['newsidakend'] as $key => &$value) {
            if (isset($value['A'])) {
                unset($value['A']);
            }
            foreach ($value as $key1 => &$value1) {
                // Check if the sub-array is an array and if 'est' key exists
                if (is_array($value1) && isset($value1['est'])) {
                    unset($value1['est']);
                }
            }
            unset($value1);
        ...
        foreach ($rekap_mutubuah['mutu_buah'] as $key => &$value) {
            // Unset the 'A' key if it exists
            if (isset($value['A'])) {
                unset($value['A']);
            }
        ...
        $rekap_mutubuah_afdeling = $rekap_mutubuah['mutu_buah'];
        $rekap_tph_afdeling = $rekap_tph['newsidakend'];

        // dd($rekap_mutubuah_afdeling, $rekap_tph_afdeling, $rekap_qc_afdeling);

        $rekap_all_data = [];
        foreach ($rekap_mutubuah_afdeling as $key => $value) {
            foreach ($value as $key1 => $value1) {
                $sum = 0;
                $get_sum = [];
                foreach ($value1 as $key2 => $value2) {
                    // Initialize variables to hold scores
                    $skor_mtb = 0; // Total score from the first array
                    $skor_mt = 0;  // Total score from the second array
                    $skor_ma = 0;  // Total score from the third array

                    // Check if the key exists in each array and accumulate scores
                    if (isset($rekap_mutubuah_afdeling[$key][$key1][$key2]['total_score'])) {
                        $skor_mtb += $value2['total_score'];
                    }
                    if (isset($rekap_tph_afdeling[$key][$key1][$key2]['total_score'])) {
                        $skor_mt += $rekap_tph_afdeling[$key][$key1][$key2]['total_score'];
                    }
                    if (isset($rekap_qc_afdeling[$key][$key1][$key2]['totalskorinspeksi'])) {
                        $skor_ma += $rekap_qc_afdeling[$key][$key1][$key2]['totalskorinspeksi'];
                    }
                    ...
                    $rekap_all_data[$key][$key1][$key2] = [
                        ...
                        'skor_mt' => $skor_mt,
                        'skor_ma' => $skor_ma,
                        'total' => $sum != 0 ? ($skor_mtb + $skor_mt + $skor_ma) / $sum : 0,  // Ensure $sum is not zero before division
                        'asistenafd' => get_nama_asisten($key1, $key2),
                        'EM' => get_nama_em($key1),
                    ];
                }
            }
        ...
        $arr['rekap_afdeling'] = $rekap_all_data;
